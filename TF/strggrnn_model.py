...
class strggrnn(tf.Module):
    def __init__(self, inputs):
        ...
        # with tf.compat.v1.variable_scope('nghood_init', reuse=True):
            # self.out_init = tf.zeros(dtype=tf.float32, shape=(args.obs_len, args.obs_len))
    ...
    def fit(self, infer=False):
        ...
        ctxt_img_pd = tf.convert_to_tensor(tf.pad(ctxt_img, paddings=tf.constant([[1, 1, ], [0, 1], [0, 0]])), dtype=tf.float32)
        ...
        graph_t = self.onlinegraph.ConstructGraph(current_batch=self.batch_coords, framenum=0,
                                                  future_traj=self.batch_future_coords, valid=infer)
        ...
        f_hat, new_hidden_state, ng_output, c_hidden_state = self.nghood_enc(inputs=inputs,
        ...
        if infer:
            self.krnl_mdl.lambda_reg, self.krnl_mdl.weight_c, self.krnl_mdl.weight_v, self.krnl_mdl.bias_v, self.krnl_mdl.ngh, \
            _, _, _, _ = self.bst_krnl_mdl_vals
